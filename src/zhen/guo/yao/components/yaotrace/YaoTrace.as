package zhen.guo.yao.components.yaotrace 
{
	...
	public class YaoTrace
	{
		...
		private static var _data:YaoData;
		
		private static var _controller:YaoController;//控制条
		private static var _view:YaoView;//内容
		...
		private static function btnClickHandler(evn:YaoBtnClickEvent):void
		{
			...
			switch(evn.btnType)
			{
				...
				case YaoTrace.ALL:
					...
					var n:uint = YaoData.msgArray.length;
					
					for (var i:uint = 0; i < n; i++ )
					{
						if (evn.btnType == YaoTrace.ALL)
						{
							_view.add(YaoData.msgArray[i][0],YaoData.msgArray[i][1]);
						}
						else
						{
							if (YaoData.msgArray[i][1] == evn.btnType)
							{
								_view.add(YaoData.msgArray[i][0],evn.btnType);
							}
						}
					}
					break;
				case YaoTrace.CLEAR:
					...
					_data.clearMsg();
					break;
				case YaoTrace.NEXT_PAGE:
					_view.rollDown();
					break;
				case YaoTrace.PREV_PAGE:
					_view.rollUp();
					break;
			}
		}
		...
		{
			var msgType:String = YaoData.msgArray[YaoData.msgArray.length - 1][1];
			
			if (msgType == _currentType||_currentType==YaoTrace.ALL)
			{
				
				_view.add(YaoData.msgArray[YaoData.msgArray.length-1][0],msgType);
			}
		...
		{
			YaoData.areaWidth = areaWidth;
			YaoData.areaHeight = areaHeight;
			...
			_frame = creatFrame(YaoData.areaWidth, YaoData.areaHeight);
			_container.addChild(_frame);
			
			_controller = new YaoController();
			_controller.addEventListener(YaoBtnClickEvent.BTN_CLICK, btnClickHandler);
