package zhen.guo.yao.components.yaotrace 
{
	 ...
	 * @author yaoguozhen
	 */
	internal class YaoView extends Sprite
	{
		 ...
		 * @param	h 高度
		 */
		public function YaoView() :void
		{
			_content = new TextField();
			
			_content = new TextField();
			_content.width = YaoData.areaWidth;
			_content.height = YaoData.areaHeight-BTN_HEIGHT;
            _content.multiline = true;
			_content.wordWrap = true; 
			addChild(_content);
			
			_mask = getMask(YaoData.areaWidth, YaoData.areaHeight - BTN_HEIGHT);
			_content.mask = _mask;
			addChild(_mask);
